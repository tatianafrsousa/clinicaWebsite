...
// Verifica se o parâmetro especialidade foi passado
if (isset($_GET['especialidade'])) {
    $id_especialidade = $_GET['especialidade'];
    ...
    $sql = "SELECT medicos.nome, medicos.apelido, medicos.contato, medicos.email, especialidades.nome_especialidade 
            ...
            WHERE medicos.id_especialidade = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('i', $id_especialidade);
    $stmt->execute();
    $result = $stmt->get_result();
} else {
    // Caso não haja especialidade, exibe todos os médicos
    $sql = "SELECT medicos.nome, medicos.apelido, medicos.contato, medicos.email, especialidades.nome_especialidade 
            FROM medicos 
            JOIN especialidades ON medicos.id_especialidade = especialidades.id_especialidade";
...
<body>
    ...
    <div class="container mt-5">
        <h2 class="text-center mb-4">Médicos Disponíveis</h2>
        <div class="row">
            <?php
            if ($result->num_rows > 0) {
                ...
                echo "<p class='text-center'>Nenhum médico encontrado para esta especialidade.</p>";
            }
            ?>
        </div>
    </div>
